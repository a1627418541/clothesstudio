...
describe("personal try-on compiler", () => {
  it("builds a versioned structured prompt and serializes fixed sections deterministically", () => {
    ...
    expect(PERSONAL_TRY_ON_COMPILER_VERSION).toBe(2);
    expect(prompt).toContain("[REFERENCE ROLES]");
    expect(prompt).toContain("first reference image is the immutable base photograph");
    expect(prompt).toContain("edit this exact photograph and this exact person");
    expect(prompt).toContain("second reference image is facial identity reference only");
    expect(prompt).toContain("[IDENTITY PRESERVATION]");
    expect(prompt).toContain("do not create a new person");
    expect(prompt).toContain("do not replace the subject");
    expect(prompt).toContain("preserve ethnicity");
    ...
    expect(prompt).toContain("preserve exact height and body proportions");
    expect(prompt).toContain("preserve shoulder width, torso length and leg length");
    expect(prompt).toContain("preserve pose, camera angle, framing and background");
    expect(prompt).toContain("do not idealize or create a fashion-model body");
    expect(prompt).toContain("no taller, slimmer or more muscular transformation");
    expect(prompt).toContain("[EDIT SCOPE]");
    expect(prompt).toContain("change clothing and footwear only");
    expect(prompt).toContain("no new sunglasses or unrelated accessories");
    expect(prompt).toContain("no editorial reposing");
    expect(prompt).toContain("no background replacement");
