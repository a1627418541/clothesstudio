...
export function parseArguments(argv: string[]): { manifest: string; provider: ProviderSelection } {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else throw new SafeCliError("Invalid benchmark arguments");
  }
  if (!manifest || !provider || !["tencent", "volcengine", "all"].includes(provider)) {
    throw new SafeCliError("Usage: npm run benchmark:try-on -- --manifest <json> --provider tencent|volcengine|all");
  }
  return { manifest, provider };
}

export function selectedProviderNames(selection: ProviderSelection): BenchmarkProviderName[] {
  return selection === "all" ? ["tencent", "volcengine"] : [selection];
}

export function assertCredentials(
  names: BenchmarkProviderName[],
  ...
  if (missing.length > 0) {
    throw new SafeCliError(`Missing try-on environment variables: ${missing.join(", ")}`);
  }
}

export function benchmarkExitCode(results: PersistedBenchmarkResult[]): 0 | 1 {
  return results.some((result) => result.status === "SUCCEEDED") ? 0 : 1;
}
...
export function isDirectExecution(
  ...
  return Boolean(executablePath) && moduleUrl === pathToFileURL(resolve(executablePath)).href;
}
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  process.exitCode = benchmarkExitCode(results);
}

if (isDirectExecution()) {
  void main().catch((error: unknown) => {
    console.error(error instanceof SafeCliError ? error.message : "Benchmark failed");
    process.exitCode = 1;
  });
}
